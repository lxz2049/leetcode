...
class Solution(object):
    def isBipartite(self, graph):
        ...
        def traverse(node):
            if graph[node] and colors[node] is None:
                colors[node] = True
                flag = False
                q = [node]
                while q:
        ...
        ret = all(traverse(i) for i in xrange(len(graph)))
        return ret

    def test(self):
        print self.isBipartite([[1,3], [0,2], [1,3], [0,2]])
        print self.isBipartite([[1,2,3], [0,2], [0,1,3], [0,2]])
