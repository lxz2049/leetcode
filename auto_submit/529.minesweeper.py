...
class Solution(object):
    def updateBoard(self, board, click):
        ...
        x, y= click
        invalid_click = lambda x, y: x < 0 or y < 0 or x >= len(board) or y >= len(board[0])

        if board[x][y] == 'E':
            count = 0
            for x1, y1 in itertools.product((x-1, x, x+1), (y-1, y, y+1)):
                if not invalid_click(x1, y1) and (board[x1][y1] == 'M' or board[x1][y1] == 'X'):
                    count += 1
            if count > 0:
                board[x][y] = str(count)
            else:
                board[x][y] = 'B'
                for x1, y1 in itertools.product((x-1, x, x+1), (y-1, y, y+1)):
                    if not invalid_click(x1, y1):
                        board = self.updateBoard(board, (x1, y1))
        elif board[x][y] == 'M':
            board[x][y] = 'X'
    ...
    def test(self):
        #print self.updateBoard([['B','1','E','1','B'],['B','1','M','1','B'],['B','1','1','1','B'],['B','B','B','B','B']], (1, 2))
