...
class Solution(object):
    def findDuplicate(self, paths):
        ...
        for p in paths:
            ...
            for f in files[1:]:
                ...
                fname = f[:div]
                content = f[div:]
                file_map[content].append("/".join((root, fname)))
    ...
    def test(self):
        input = ["root/a 1.txt(abcd) 2.txt(efgh)",
                 ...
                 "root/c/d 4.txt(efgh)",
                 "root 4.txt(efgh)"]
        print [["root/a/2.txt","root/c/d/4.txt","root/4.txt"], ["root/a/1.txt","root/c/3.txt"]] ,self.findDuplicate(input)
        print self.findDuplicate(["root/a 1.txt(abcd) 2.txt(efsfgh)","root/c 3.txt(abdfcd)","root/c/d 4.txt(efggdfh)"])
