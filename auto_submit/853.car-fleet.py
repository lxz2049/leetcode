...
class Solution(object):
    def carFleet(self, target, position, speed):
        ...
        :rtype: int
        """
        time_position = [(float(target - p) / s, p) for p, s in zip(position, speed)]
        ret = 0
        last_position = -1
        for time, position in sorted(time_position, reverse=True):
            #print time, position
    ...
    def test(self):
        print self.carFleet(12, [10,8,0,5,3], [2,4,1,1,3])
