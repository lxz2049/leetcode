...
public:
    ...
    vector<vector<string>> accountsMerge(vector<vector<string>>& accounts) {
        ...
        for (int i=0; i<accounts.size(); ++i) {
            ...
            for (int j=1; j<accounts[i].size(); ++j) {
                ...
                if (disjointSet.find(email) == disjointSet.end()) {
                    disjointSet[email] = pair<string, int>(email, 1);
                }
                accountMapping[email] = name;
        ...
        for (unordered_map<string, pair<string, int>>::iterator it = disjointSet.begin(); it != disjointSet.end(); it++) {
            string parent = find(it->first);
            ans[parent].insert(it->first);
            //cout<<"parent: "<<parent<<" email: "<<it->first<<endl;
        }
    ...
    void test() {
        vector<vector<string>> input = {{"a", "email"}, {"a", "email", "another_email"}};
