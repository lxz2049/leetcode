...
class Solution:
    def escapeGhosts(self, ghosts: List[List[int]], target: List[int]) -> bool:
        getEscapeTime = lambda pos: sum(abs(t-p) for t, p in zip(target, pos)) 
        return all(getEscapeTime([0,0]) < getEscapeTime(ghost) for ghost in ghosts)
    def test(self):
        print(self.escapeGhosts([[1,0],[0,3]], [0,1]))
        print(self.escapeGhosts([[1,0]], [2,0]))
        print(self.escapeGhosts([[2,0]], [1,0]))
