...
class Solution(object):
    def orderOfLargestPlusSign(self, N, mines):
        ...
        grid = [[[0, 0, 0, 0] for _ in xrange(N) ] for _ in xrange(N)]
        for i in xrange(N):
            for j in xrange(N):
                if (i, j) not in mines:
                    grid[i][j][0] = 1 + (grid[i-1][j][0] if i else 0)
                    grid[i][j][1] = 1 + (grid[i][j-1][1] if j else 0)
        
        for i in reversed(range(N)):
            for j in reversed(range(N)):
                if (i, j) not in mines:
                    grid[i][j][2] = 1 + (grid[i+1][j][2] if i < N-1 else 0)
                    grid[i][j][3] = 1 + (grid[i][j+1][3] if j < N-1 else 0)
        return max(min(cell) for row in grid for cell in row)

    def test(self):
        print self.orderOfLargestPlusSign(5, [[4, 2]])
        print self.orderOfLargestPlusSign(2, [])
        print self.orderOfLargestPlusSign(1, [[0, 0]])
