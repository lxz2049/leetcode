...
class Solution(object):
    def PredictTheWinner(self, nums):
        ...
        def traverse(lo, hi):
            ...
            dp[lo][hi] = max(nums[lo] - traverse(lo+1, hi), nums[hi] - traverse(lo, hi-1))
            return dp[lo][hi]

        return traverse(0, len(nums)-1) >= 0

    def test(self):
        print self.PredictTheWinner([1,5,2])
        print self.PredictTheWinner([1,5,233,7])
