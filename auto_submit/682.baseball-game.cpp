...
public:
    ...
    int calPoints(vector<string>& ops) {
		...
		for (int i=0; i<ops.size(); ++i) {
			if(ops[i] == "C")
				...
				pts[head] = pts[head-1] * 2;
				head++;
			}
			else if(ops[i] == "+") {
				pts[head] = pts[head-1] + pts[head-2];
				head++;
			}
			else {
				pts[head] = atoi(ops[i].c_str());
				head++;
			}
		}
		int ans = 0;
	...
	bool test() {
		...
		vector<string> v1(s1, s1+5);
		vector<string> v2(s2, s2+8);
		return calPoints(v1) == 30 && calPoints(v2) == 27;
	}
};
