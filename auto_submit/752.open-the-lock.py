...
class Solution(object):
    def openLock(self, deadends, target):
        ...
        :rtype: int
        """
        deadends = set((int(s) for s in deadends))
        target = int(target)
        visited = set()
        s = deque([(0, 0, 0)])
        while s:
            parent, state, turn = s.pop()

            if state in deadends or state in visited:
                continue
            #print " " * turn, "state", "%04d %04d" % (parent, state)
            ...
            s.appendleft((state, state / 10 * 10 + (state + 1) % 10, turn+1))
            s.appendleft((state, state / 10 * 10 + (state - 1) % 10, turn+1))

            s.appendleft((state, state / 100 * 100 + (state + 10) % 100 , turn+1))
            s.appendleft((state, state / 100 * 100 + (state - 10) % 100, turn+1))
            
            s.appendleft((state, state / 1000 * 1000 + (state + 100) % 1000, turn+1))
            s.appendleft((state, state / 1000 * 1000 + (state - 100) % 1000, turn+1))

            s.appendleft((state, (state + 1000) % 10000, turn+1))
            s.appendleft((state, (state - 1000) % 10000, turn+1))

        return -1

    def test(self):
        print self.openLock(["8888"], "0009")
        print self.openLock(["0201", "0101", "0102", "1212", "2002"], "0202")
        print self.openLock(["8888"], "0009")
        print self.openLock(["0000"], "8888")
