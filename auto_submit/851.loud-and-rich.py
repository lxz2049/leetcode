...
class Solution(object):
    def loudAndRich(self, richer, quiet):
        ...
        :rtype: List[int]
        """
        richGraph = [[] for _ in quiet]
        for rich, poor in richer:
            richGraph[poor].append(rich)

        visited = [False] * len(quiet)
        quietness = [None] * len(quiet)
        def traverse(i):
            if not quietness[i]:
                quietness[i] = (quiet[i], i)
                for rich in richGraph[i]:
                   quietness[i] = min(quietness[i], traverse(rich))
            return quietness[i]

        return [traverse(i)[1] for i in xrange(len(quiet))]
